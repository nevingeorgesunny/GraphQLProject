...
public class MongoGraphQLFetchContext {
    ...
    public static MongoGraphQLFetchContext build(DataFetchingEnvironment dataFetchingEnvironment) {
        ...
        try {
            criteria = filterExpression.getExpression(ExpressionFormat.MONGO);
        }catch (Exception e){
            //Some issue. log it exception while criteria created, so this will be considered as no filters
        }
        ...
        Pagination pagination = paginationMap == null ?
                Pagination.builder()
                        .pageSize(10)
                        .offset(0)
                        .build()
